...
void main() {
  ...
  vec3 colorComponent = clamp(clamp(texture(texture0, uv0).rgb*(vec3(texture(texture1, uv1).a).g*65280 + vec3(texture(texture1, uv1).a).r*255 > 32896 ? vec3(1) : vec3(0)), 0, 1) + texture(texture2, uv2).rgb*(vec3(1) + vec3(-1)*(vec3(texture(texture1, uv1).a).g*65280 + vec3(texture(texture1, uv1).a).r*255 > 32896 ? vec3(1) : vec3(0))) + clamp((color_GxColorRegister5 + texture(texture3, uv0).rgb*(vec3(1) + vec3(-1)*vec3(0.625)) + color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + color_GxAmbientColor0), 0, 1)*vec3(0.625) + vec3(-0.5)), 0, 1)*(vec3(1) + vec3(-1)*vec3(0.30980392156862746,0.4235294117647059,0.3803921568627451)) + vec3(-0.5), 0, 1);
